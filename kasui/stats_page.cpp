...
void
stats_page_item::draw_frame(const g2d::mat4& proj_modelview, float alpha) const
{
	...
	const float x0 = left_border, x1 = left_border + border_radius, x2 = window_width - border_radius, x3 = window_width;
...
: stats_page_item(g2d::texture_manager::get_instance().load("images/b-button-border.png"))
{
	...
	text_
		.text_program(get_program_instance<program_text>().get_raw())
		.push_matrix()
...
void
kanji_info_item::draw(const g2d::mat4& proj_modelview, float alpha) const
{
...
void
jukugo_info_item::reset_contents()
{
	const g2d::font *micro_font = g2d::font_manager::get_instance().load("fonts/micro");
	...
	hits_text_
		...
		.align_right().render_text(micro_font, L"%d", jukugo_->hits);
}

void
jukugo_info_item::draw(const g2d::mat4& proj_modelview, float alpha) const
{
	draw_frame(proj_modelview, alpha);
...
void
stats_page::draw(const g2d::mat4& mat, float alpha) const
{
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
void
stats_page::draw_title(const g2d::mat4& mat, float alpha) const
{
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
void
stats_page::on_drag_end()
{
	speed_ = total_drag_dy_/(tics_ - touch_start_tic_);
}

void
stats_page::update_y_offset(float dy)
{
...
void
stats_state_impl::draw_pages(float x_offset) const
{
...
void
stats_state_impl::draw_page(const stats_page *page, float x_offset) const
{
...
void
stats_state_impl::update(uint32_t dt)
{
...
int
stats_state_impl::get_next_level() const
{
...
void
stats_state_impl::on_touch_down(float x, float y)
{
...
void
stats_state_impl::on_touch_move(float x, float y)
{
	if (pause_button_.on_touch_down(x, y))
		return;
...
void
stats_page_state::reset()
{
	impl_->reset();
}

void
stats_page_state::redraw() const
{
	impl_->redraw();
}

void
stats_page_state::update(uint32_t dt)
{
	impl_->update(dt);
}

void
stats_page_state::on_touch_down(float x, float y)
{
	impl_->on_touch_down(x, y);
}

void
stats_page_state::on_touch_up()
{
	impl_->on_touch_up();
}

void
stats_page_state::on_touch_move(float x, float y)
{
	impl_->on_touch_move(x, y);
}

void
stats_page_state::on_back_key()
{
	impl_->on_back_key();
}

void
stats_page_state::on_menu_key()
{
	impl_->on_menu_key();
}
