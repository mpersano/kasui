...
static void dump_frames_init(int width, int height)
{
...
{
    glReadPixels(0, 0, viewport_width, viewport_height, GL_RGB, GL_UNSIGNED_BYTE, rgb_buffer);
    ...
    int out_size = avcodec_encode_video(codec_ctx, out_buf, sizeof out_buf, frame);
    fwrite(out_buf, 1, out_size, out_fp);
}

static void dump_frames_end()
{
    int out_size;
...
static void handle_events(kasui &k)
{
    SDL_Event event;
