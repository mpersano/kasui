...
public:
	...
	int get_num_level_block_types() const
	{ return num_level_block_types_; }

	float get_width() const
	{ return cols_*cell_size_; }

	float get_height() const
	{ return rows_*cell_size_; }

	int get_block_at(int row, int col) const
	{ return grid_[row*cols_ + col]; }

	void set_block_at(int row, int col, int value)
	{ grid_[row*cols_ + col] = value; }

	void set_block_kanji_at(int row, int col, wchar_t kanji);

	int get_block_type_at(int row, int col) const
	{ return get_block_at(row, col) & ~BAKUDAN_FLAG; }

	int get_jukugo_left() const
	{ return level_jukugo_left_; }

	int get_score() const
	{ return score_; }

	bool is_in_falling_block_state() const
	{ return cur_state_ == STATE_FALLING_BLOCK; }

	bool can_consume_gestures() const
	{ return cur_state_ == STATE_FALLING_BLOCK || cur_state_ == STATE_HINT; }

	bool is_in_game_over_state() const
	{ return cur_state_ == STATE_GAME_OVER; }

	bool is_in_level_completed_state() const
	{ return cur_state_ == STATE_LEVEL_COMPLETED; }
	...
	void set_event_listener(world_event_listener *listener)
