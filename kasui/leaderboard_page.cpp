...
void
draw_message(const g2d::mat4& mat, float alpha, const wchar_t *text)
{
...
void
item::draw(const g2d::mat4& mat, float alpha) const
{
	glEnable(GL_BLEND);
...
bool
leaderboard_page::is_hiscore(int score) const
{
	return items_.size() < MAX_HISCORES || score > items_.back()->get_score();
}

bool
leaderboard_page::load_cache()
{
...
void
leaderboard_page::save_cache() const
{
...
void
leaderboard_page::draw_title(const g2d::mat4& mat, float alpha) const
{
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
void
leaderboard_page::draw_items(const g2d::mat4& mat, float alpha) const
{
...
void
leaderboard_page::update(uint32_t dt)
{
	tics_ += dt;
...
void
leaderboard_page::update_y_offset(float dy)
{
	const float top_y = window_height - TITLE_HEIGHT;
	const float total_height = items_.size()*item::HEIGHT;
...
void
leaderboard_page::on_drag_start()
{
...
void
leaderboard_page::on_drag_end()
{
	speed_ = total_drag_dy_/(tics_ - touch_start_tic_);
}

void
leaderboard_page::on_drag(float dy)
{
	total_drag_dy_ += dy;
	update_y_offset(dy);
}
...
void
local_leaderboard::draw(const g2d::mat4& mat, float alpha) const
{
...
bool
local_leaderboard::async_check_hiscore(leaderboard_event_listener *listener, int score)
{
...
bool
local_leaderboard::async_insert_hiscore(leaderboard_event_listener *listener, const wchar_t *name, int score)
{
...
void
net_leaderboard::draw(const g2d::mat4& mat, float alpha) const
{
	switch (state_) {
		case STATE_LOADING:
...
void
net_leaderboard::answer_hiscore_requests()
{
	assert(state_ == STATE_DISPLAYING || state_ == STATE_ERROR);

	std::for_each(
		check_hiscore_requests_.begin(),
		check_hiscore_requests_.end(),
...
void
net_leaderboard::on_request_error()
{
	state_ = STATE_ERROR;
...
void
net_leaderboard::on_request_completed(int status, const char *content, size_t content_len)
{
	if (status != 200) {
		state_ = STATE_ERROR;
		answer_hiscore_requests();
...
void
net_leaderboard::draw_loading_message(const g2d::mat4& mat, float alpha) const
{
	draw_message(mat, alpha, L"loading leaderboard...");
}

void
net_leaderboard::draw_error_message(const g2d::mat4& mat, float alpha) const
{
...
bool
net_leaderboard::async_check_hiscore(leaderboard_event_listener *listener, int score)
{
	if (need_refresh())
		start_loading();
...
bool
net_leaderboard::async_insert_hiscore(leaderboard_event_listener *listener, const wchar_t *name, int score)
{
...
get_net_leaderboard()
{
	...
	// static net_leaderboard instance("top scores", "leaderboard-net", "http://localhost/leaderboard.php");
	return instance;
}
