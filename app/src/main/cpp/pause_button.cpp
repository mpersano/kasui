...
void pause_button::reset()
{
    is_selected = false;
}

void pause_button::set_callback(std::function<void(void)> callback)
{
    callback_ = callback;
}

void pause_button::draw(float alpha) const
{
...
bool pause_button::on_touch_down(float x, float y)
{
    return is_selected = x >= x_base && x < x_base + SIZE && y >= y_base && y < y_base + SIZE;
}

bool pause_button::on_touch_up()
{
    if (!is_selected) {
        return false;
    } else {
